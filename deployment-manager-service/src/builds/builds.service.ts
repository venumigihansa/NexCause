...
export class BuildsService {
  ...
  async create(appId: string, createBuildDto: CreateBuildDto = {}) {
    ...
    if (!app.repoUrl) {
      throw new BadRequestException('Git apps must have a repoUrl before building');
    }

    const buildId = randomUUID();
    const namespace =
      this.configService.get<string>('defaultNamespace') ?? 'apps';
    ...
    const labels = this.kubernetesService.buildJobLabels(app.id, buildId);
  ...
  async getLogs(id: string) {
    ...
    const namespace =
      this.configService.get<string>('defaultNamespace') ?? 'apps';
    const labels = this.kubernetesService.buildJobLabels(build.appId, build.id);
